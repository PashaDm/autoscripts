define(['./utils/ui/login/login'], function (loginHelper) {
    ...
    return function (task) {
        ...
        task('Add card', function (task) {
            ...
            task.step('Open deposit page', function(done) {
                       task.ui.isElementPresent('.fn-open-menu', function(isPresent){
                              if(isPresent) {
                                 task.ui
                                  .click('.fn-open-menu')
                                  .waitForElement('li[data-url="/payment/deposit"] a')
            ...
            task.step('Check deposit portlet', function(done) {
                      task.ui
            ...
            task.step('AcceptSD', function (done) {
             ...
             task.step('Accept bonus popup if present', function (done) {
                        var bonusPopup = false;
                        task.ui
                         .pause(2000)
                         .waitForElement('.fn-accept', function (isPresent) {
                           if (isPresent) {
                             task.ui
                              .click('.fn-accept')
                              .end(done);
